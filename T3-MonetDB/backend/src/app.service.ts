...
export class DatabaseService implements OnModuleInit {
  ...
  async cargarCsvMasivo() {
    ...
    return { message: '5,000,000 filas importadas con éxito' };
  }

  async obtenerDatosDashboard(filtros: any) {
    let whereClause = 'WHERE 1=1';
    
    if (filtros.ciudad) whereClause += ` AND ciudad = '${filtros.ciudad}'`;
    if (filtros.categoria) whereClause += ` AND categoria = '${filtros.categoria}'`;
    if (filtros.fecha) whereClause += ` AND fecha = '${filtros.fecha}'`;
    if (filtros.metodopago) whereClause += ` AND metodopago = '${filtros.metodopago}'`;
    ...
    const mapChartData = (res: any, key1: string, key2: string) => {
      if (!res?.data) return [];
      return res.data.map((row: any) => ({ [key1]: row[0], [key2]: parseFloat(row[1] || 0) }));
    };
